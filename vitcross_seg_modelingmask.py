...
class Attention(nn.Module):
    ...
    def forward(self, hidden_statesx, hidden_statesy):
        batch_size = hidden_statesx.size(0)
        height = 16
        width = 16
        ...
        if self.mask is None or self.mask.size(-1) != height * width:
            binary_mask = torch.randint(0, 2, (batch_size, height, width), device=hidden_statesx.device)
            binary_mask = binary_mask.view(batch_size, -1)
            processed_mask = torch.where(binary_mask > 0.5, torch.tensor(0.0, device=hidden_statesx.device),
                                         torch.tensor(-float('inf'), device=hidden_statesx.device))
            self.mask = processed_mask.unsqueeze(1).unsqueeze(1).expand(batch_size, self.num_attention_heads, height * width, height * width)
        ...
        if self.mask1 is None or self.mask1.size(-1) != height * width:
            binary_mask1 = torch.randint(0, 2, (batch_size, height, width), device=hidden_statesx.device)
            binary_mask1 = binary_mask1.view(batch_size, -1)
            processed_mask1 = torch.where(binary_mask1 > 0.5, torch.tensor(0.0, device=hidden_statesx.device),
                                         torch.tensor(-float('inf'), device=hidden_statesx.device))
            self.mask1 = processed_mask1.unsqueeze(1).unsqueeze(1).expand(batch_size, -1, height * width, height * width)

        attention_scores = attention_scores + self.mask1
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if self.hybrid:
            self.hybrid_model = FuseResNetV2(block_units=config.resnet.num_layers,
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids, dsm_ids):
        embeddingsx, embeddingsy, features = self.embeddings(input_ids,
                                                             dsm_ids)
        encodedx, encodedy, attn_weights = self.encoder(embeddingsx,
                                                        embeddingsy)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, y):
        x, y, attn_weights, features = self.transformer(x, y)
        x = x + y
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if self.transformer.embeddings.hybrid:
                ...
                pretrained_weight = np2th(ws, conv=True)
                new_weight = torch.zeros((64, 4, 7, 7))
                new_weight[:, :3, :, :] = pretrained_weight
                new_weight[:, 3, :, :] = pretrained_weight.mean(dim=1)
                self.transformer.embeddings.hybrid_model.root.conv.weight.copy_(new_weight)
